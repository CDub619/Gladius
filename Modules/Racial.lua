...
local unitRaceCDs = {
	...
	["ORC"] = { cooldown = 120, spellID = 33702, sharesCD = false },
...
--[[function Racial:ARENA_COOLDOWNS_UPDATE(event, unit)
	...
	if duration then
		if duration == 120000 then
			--print(unit.." trinket "..race)
			--self:UpdateRacial(unit, sharedCD)
		elseif duration == 30000 then
			--print(unit.." dwarf or undead or darkdwarf "..race)
		elseif duration == 90000 then
			--print(unit.." human "..race)
		elseif duration == 60000 then
			--print(unit.." adapted "..race)
		end
	end
end]]
...
function Racial:UNIT_AURA(event, unit)
	...
	if UnitBuff(unit, "Blood Fury") then
		local _,_,_,_,_,t = UnitBuff(unit, "Blood Fury")
		local g = t-GetTime()
		if g then
...
function Racial:UNIT_SPELLCAST_SUCCEEDED(event, unit, spellLineID, spell)
	...
	if unitRaceCDs[race].sharesCD then
		...
		if (cd < sharedCD) then
			-- PVP Trinkets
			if spell == 42292 or spell == 336126 then
				self:UpdateRacial(unit, sharedCD)
			end
			-- Honorable Medallion
